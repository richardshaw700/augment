...
class ComputerTool(BaseAnthropicTool):
    ...
    def __init__(self):
        ...
        MAX_WIDTH = 800  # Reduced from 1280 to 800 - saves ~60% on image tokens
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        print(
            f"[{timestamp}] ### Performing action: {action}{f", text: {text}" if text else ''}{f", coordinate: {coordinate}" if coordinate else ''}"
        )

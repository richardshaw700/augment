...
class UIInspectorApp {
    ...
    func run() {
        ...
        var finalElements = filteredElements
        ...
        // Step 9: Menu Bar Context Inspection
        print("\n🖥️ MENU BAR CONTEXT")
        print("===================")
        let menuBarStart = Date()
        let menuBarContext = MenuBarInspector.inspectMenuBar()
        let menuBarTime = Date().timeIntervalSince(menuBarStart)
        stepTimes.append(("Menu bar context", menuBarTime))
        
        if let activeApp = menuBarContext["activeApplication"] as? [String: Any],
           let appName = activeApp["name"] as? String {
            print("🎯 Active application: \(appName)")
        }
        
        if let menuItems = menuBarContext["menuItems"] as? [[String: Any]] {
            print("📋 Menu items: \(menuItems.count) items")
            
            // Convert menu bar items to UIElements and add to final elements
            let menuBarElements = convertMenuBarItemsToUIElements(menuItems)
            finalElements.append(contentsOf: menuBarElements)
            print("🔗 Integrated \(menuBarElements.count) menu bar elements")
        }
        ...
        // Save files with automatic cleanup
        ...
        do {
            let paths = try fileManager.saveFiles(
                jsonData: jsonData,
        ...
        // Output JSON to stdout for GPT engine consumption
    ...
    private func convertMenuBarItemsToUIElements(_ menuItems: [[String: Any]]) -> [UIElement] {
        var elements: [UIElement] = []
        
        for item in menuItems {
            guard let title = item["title"] as? String,
                  let menuBarPosition = item["menuBarPosition"] as? String else {
                continue
            }
            ...
            // Use menu bar coordinate instead of pixel coordinates
            let element = UIElement(
                id: UUID().uuidString,
                type: elementType,
                position: CGPoint(x: 0, y: 0), // Menu bar items don't use pixel coordinates
                size: CGSize(width: 60, height: 24), // Standard menu bar item size
                accessibilityData: nil,
                ocrData: nil,
                isClickable: true,
